...
parser.add_argument("--model_names", default=['PIP'], type=list, help="'PIP', 'SCTransNet', 'RDIAN'")
parser.add_argument("--dataset_names", default=['NUAA-SIRST'], type=list)#  NUAA-SIRST NUDT-SIRST IRSTD-1K
...
parser.add_argument("--save", default='/home/xxx/IOVarNet/log', type=str, help="Save path of checkpoints")



parser.add_argument("--log_dir", type=str, default="/home/xxx/IOVarNet/otherlogs/", help='path of log files')
...
def train():
    ...
    if opt.optimizer_name == 'Adam':
        opt.optimizer_settings = {'lr': 0.001}###  NUAA/NUDT: 1e-3; 1K: 1e-4
...
def Test(save_pth):
    ...
    with torch.no_grad():
        ...
        for idx_iter, (img, gt_mask, size, _) in enumerate(test_loader):
            img = Variable(img).cuda()
            pred, _, _, _, _, _, _, _, _, _, _, _, _ = net.forward(img)
